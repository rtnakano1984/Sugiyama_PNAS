...
# Fig 5d (for statistical analysis)
...
dat_all <- read.table(paste(data_dir, "R_Fig 5d.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
dat_all$batch <- factor(dat_all$batch)
dat_all$time  <- str_replace_all(dat_all$time, " ", "_")
...
for(cpd in unique(dat_all$cpd)){
  ...
  lmer_fit <- lmer(sqrt(value) ~ time:genotype - 1 + (1|batch), dat)
  ...
  write.table(data.frame(group_comp=group_comp, p.val=p.val, adj_p=adj_p.val), paste(data_dir, cpd, "-P_values.txt", sep=""), col.names=T, row.names=F, quote=F, sep="\t")
  ...
  names(estim) <- str_replace_all(names(estim), "time", "")
  ...
  estim_df_temp$logFC <- log2(estim_df_temp$estim / estim_df_temp$estim[estim_df_temp$group == "S1500_0d:wt"])
...
# boxplot
p <- ggplot(dat_all, aes(x=genotype, y=value, group=genotype, shape=factor(batch))) +
  geom_boxplot(outlier.shape=NA, position=position_dodge()) +
  geom_point(, position=position_jitterdodge()) +
  facet_grid(cpd ~ time, scales="free") +
  theme_RTN +
