...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_Fig 3a/"

dat_all <- read.table(paste(data_dir, "R_Fig 3a.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
dat_all$genotype <- str_replace(dat_all$genotype, " ", "")
dat_all$batch <- factor(dat_all$batch)

genotype <- c("wtintact", "wthomogenate", "medium")
dat_all$genotype <- factor(dat_all$genotype, levels=genotype)

time <- c("0h", "6min", "3h", "9h", "24h", "48h")
...
for(cpd in unique(dat_all$cpd)){  
  ...
  pdf(paste(data_dir, cpd, "-data_nomral.pdf", sep=""))
    hist(dat$relative, breaks=20)
    qqnorm(dat$relative)
    qqline(dat$relative, col='red')


    hist(sqrt(dat$relative), breaks=20)
    qqnorm(sqrt(dat$relative))
    qqline(sqrt(dat$relative), col='red')

    hist(log10(dat$relative), breaks=20)
    qqnorm(log10(dat$relative))
    qqline(log10(dat$relative), col='red')
  dev.off()

  lmer_fit <- lmer(sqrt(relative) ~ genotype:time - 1 + (1|batch), dat)
  ...
  sort <- apply(expand.grid(time, genotype)[, 2:1], 1, function(x) paste(x, collapse=":"))
