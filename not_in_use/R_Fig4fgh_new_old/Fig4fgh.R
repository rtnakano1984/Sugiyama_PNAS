...
for(x in unique(dat$substrate)){
	...
	fit <- lm(sqrt(value) ~ 0 + genotype:sulfur, temp)
	anova <- anova(aov(fit))
	...
	p <- ggplot(temp, aes(x=sulfur, y=sqrt(value), colour=genotype)) + geom_point(position=position_jitterdodge(jitter.width=.2)) + geom_boxplot(fill=NA, outlier.shape=NA) + theme_bw()
	ggsave(p, file=paste(data_dir, x, "-boxplot.pdf", sep=""), width=2.5, height=4, bg="transparent")
