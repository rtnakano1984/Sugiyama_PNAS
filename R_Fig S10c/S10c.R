...
# Fig S10c (for statistical analysis)
...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_Fig S10c/"

dat_all <- read.table(paste(data_dir, "R_Fig S10c.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
...
p <- ggplot(dat_all, aes(x=genotype, y=value, group=genotype, shape=factor(batch))) +
  ...
  facet_grid(cpd ~ sulfur, scales="free") +
  theme_RTN +
