...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_Fig 2c/"

dat <- read.table(paste(data_dir, "R_Fig 2c.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
...
pairwise_test <- function(dat_lmer){
	...
	return(p.letters$Letters[c("S1500", "S150", "4MSB", "4MSB_34S")])
}
