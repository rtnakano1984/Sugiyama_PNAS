...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_FigS8/"
...
for(x in compound){
	...
	idx <- order(-tukey$'sulfur'[, 'diff'])
	p.letters <- multcompLetters(tukey$'sulfur'[idx, 'p adj'])
	...
	p <- ggplot(temp, aes(x=sulfur, y=value)) +
