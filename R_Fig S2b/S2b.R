...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_Fig S2b/"

dat <- read.table(paste(data_dir, "R_Fig S2b.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
...
for(cpd in unique(dat$cpd)){
	...
	# lmer_fit <- lmer(lvalue ~ sulfur - 1 + (1|batch), temp)
	...
	names(p.letters$Letters) <- str_replace(names(p.letters$Letters), "sulfur", "")
	write.table(as.data.frame(p.letters$Letters[sulfur]), file=paste(data_dir, cpd, "-isotope_0-FDR_letters.txt", sep=""), sep="\t", row.names=T,col.names=NA, quote=F)
	...
	names(p.letters$Letters) <- str_replace(names(p.letters$Letters), "sulfur", "")
	write.table(as.data.frame(p.letters$Letters[sulfur]), file=paste(data_dir, cpd, "-isotope_2-FDR_letters.txt", sep=""), sep="\t", row.names=T,col.names=NA, quote=F)
