...
geno_label <- c(
	...
	expression(italic("tgg1"~"tgg2")),
	expression(italic("tgg4"~"tgg5")),
	expression(italic("pen2-3")),
	expression(italic("pyk10-1")),
	expression(italic("bglu18")))
...
for(x in unique(mean$cpd)){
	for(y in unique(mean$sulfur)){
		idx_0 <- mean$cpd == x & mean$sulfur == y
		temp <- mean[idx_0,]
		sig <- 0
		for(i in 2:nrow(temp)){
			idx <- temp$genotype == "Col"
