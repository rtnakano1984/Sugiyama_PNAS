...
data_dir <- "/netscratch/dep_psl/grp_psl/ThomasN/ryosuke_temp/R_Fig S1ac/"

dat <- read.table(paste(data_dir, "R_Fig S1ac.csv", sep=""), sep=",", header=T, stringsAsFactors=F)
dat$genotype <- str_replace(dat$genotype, " ", "")
dat$batch <- factor(dat$batch)

sulfur <- c("S1500", "S150", "S15", "S0", "4MSB")
